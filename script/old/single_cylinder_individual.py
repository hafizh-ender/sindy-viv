...
def __main__():
    ...
    for vr in vr_list:
        ...
        print(f"Performing SINDy for VR value: {vr}...")
        ...
        state_feature_library.fit(x=X)
        
        # Check terms in the overall feature library
        overall_features = state_feature_library.get_feature_names(state_variable_names)
        ...
        if args.optimizer == 'STLSQ':
            ...
            feature_hdot_index = overall_features.index('\\dot{h}_0')
            ...
            if '\\ddot{h}_0' in state_variable_names:
                feature_hddot_index = overall_features.index('\\ddot{h}_0')
                for j in range(n_features):
                    pin(target_hddot_index, j, 1.0 if j == feature_hddot_index else 0.0)
            else:
                feature_h_index = overall_features.index('h_0')
                feature_vr2cl_index = overall_features.index('C_{L0}')
                for j in range(n_features):
                    if j == feature_h_index:
                        pin(target_hddot_index, j, -4 * np.pi**2)
                    elif j == feature_vr2cl_index:
                        pin(target_hddot_index, j, +2 * vr**2 / (np.pi * MSTAR_TRUE))
                    else:
        ...
        model.fit(x=X, t=scaled_t0, feature_names=state_variable_names, x_dot=X_dot_deriv)
        ...
        sindy_results_dir = os.path.join(differentiation_save_dir, sindy_case, optimizer_foldername, state_feature_library_foldername)
        os.makedirs(sindy_results_dir, exist_ok=True)
        
        coefficients_filename = f"coefficients_single_{vr}.csv"
        ...
        fig, axs = plot_full(t0, X_dot_deriv_df, feature_names=derivative_state_variable_names, vr=vr, X_pred_df=X_dot_pred_df, xlim='auto', t_stat_start=0, nrmse_dict=semi_nrmse_dict)
        ...
        for i in range(args.num_closeups):
            if args.closeup_xlims is not None and len(args.closeup_xlims) >= 2 * (i + 1):
                closeup_xlim = args.closeup_xlims[2 * i: 2 * i + 2]
                fig, axs = plot_full(t0, X_dot_deriv_df, feature_names=derivative_state_variable_names, vr=vr, X_pred_df=X_dot_pred_df, xlim=closeup_xlim, t_stat_start=closeup_xlim[0], t_stat_end=closeup_xlim[1], nrmse_dict=semi_nrmse_dict)
    ...
    true_nrmse_filename = f"true_nrmse_single_individual.csv"
    ...
    semi_nrmse_filename = f"semi_nrmse_single_individual.csv"
