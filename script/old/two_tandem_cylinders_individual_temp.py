...
def __main__():
    ...
    for vr in vr_list:
        for lpd in lpd_list:
            ...
            print(f"Performing SINDy for VR value: {vr}, LpD value: {lpd}...")
            ...
            if args.optimizer == 'STLSQ':
                ...
                feature_hdot0_index = overall_features.index('\\dot{h}_0')
                feature_hdot1_index = overall_features.index('\\dot{h}_1')
                ...
                if '\\ddot{h}_0' in state_variable_names and '\\ddot{h}_1' in state_variable_names:
                    feature_hddot0_index = overall_features.index('\\ddot{h}_0')
                    feature_hddot1_index = overall_features.index('\\ddot{h}_1')
                    for j in range(n_features):
                    ...
                    feature_h0_index = overall_features.index('h_0')
                    feature_vr2cl0_index = overall_features.index('C_{L0}')
                    
                    feature_h1_index = overall_features.index('h_1')
                    feature_vr2cl1_index = overall_features.index('C_{L1}')
                    for j in range(n_features):
                        if j == feature_h0_index:
                            pin(target_hddot0_index, j, -4 * np.pi**2)
                        elif j == feature_vr2cl0_index:
                            pin(target_hddot0_index, j, +2 * vr**2 / (np.pi * MSTAR_TRUE))
                        else:
                        ...
                        if j == feature_h1_index:
                            ...
                            pin(target_hddot1_index, j, +2 * vr**2 / (np.pi * MSTAR_TRUE))
                        else:
            ...
            coefficients_filename = f"coefficients_two_tandem_{vr}_{lpd}.csv"
    ...
    true_nrmse_filename = f"true_nrmse_two_tandem_individual.csv"
    ...
    semi_nrmse_filename = f"semi_nrmse_two_tandem_individual.csv"
