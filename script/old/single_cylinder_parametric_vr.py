...
def plot_full(t, X_true_df, feature_names, vr, X_pred_df=None, xlim='auto', t_stat_start=0, t_stat_end=None, nrmse_dict=None):
    ...
    for i in range(len(feature_names)):
        ...
        if X_pred_df is not None and feature_name in X_pred_df.columns:
                ax.plot(t, X_pred_df[feature_name].values, label=f'${feature_name}$ (Predicted)', color='tab:orange')
...
def __main__():
    ...
    parser.add_argument(
        ...
        choices=['FiniteDifference', 'finite_difference', 'SmoothedFiniteDifference', 'savitzky_golay', 'spline', 'trend_filtered', 'spectral', 'scipy_savitzky_golay'],
    ...
    # Prepare the list for parametric SINDy (SINDyC)
    # If you change these, don't forget to adjust X and U in the loop below accordingly
    state_variable_names = [
        'h_0', '\\dot{h}_0', 'C_{L0}'
        # 'h_0', '\\dot{h}_0', '\\ddot{h}_0', 'C_{L0}'
    ]
    ...
    for vr in vr_list:
        ...
        # # Get last few seconds of data for analysis (e.g., last 400 seconds)
        # h0 = h0[t0 >= t0[-1] - 400]
        # h_dot0 = h_dot0[t0 >= t0[-1] - 400]
        # h_ddot0 = h_ddot0[t0 >= t0[-1] - 400]
        # cl0 = cl0[t0 >= t0[-1] - 400]
        # cd0 = cd0[t0 >= t0[-1] - 400]
        # t0 = t0[t0 >= t0[-1] - 400]

        # Prepare the state matrix X and input matrix U for the current VR
        X = np.vstack((
            h0, h_dot0, cl0,
            # h0, h_dot0, h_ddot0, cl0,
            # h0, h_dot0, cl0, cd0,
        )).T
        ...
        U = vrs.reshape(-1, 1)  # Reshape to be a column vector

        # Plot the true and derived derivatives for the current VR
        if args.generate_state_plots:
            fig, axs = plot_full(t0, X_df, feature_names=state_variable_names, vr=vr, xlim='auto', t_stat_start=0)
        ...
        # Append the current VR's data to the lists for parametric SINDy
        scaled_t0 = t0 / vr  # Normalize time by VR for parametric SINDy
        
        scaled_t0s.append(scaled_t0)  # Normalize time by VR
        Xs.append(X)
        Us.append(U)
        ...
        X_dot_true_df = pd.DataFrame(X_dot_true, columns=['\\dot{h}_0', '\\ddot{h}_0'])
        X_dot_true_dfs.append(X_dot_true_df)
        ...
        if args.derivative_method == 'FiniteDifference':
            ...
            smoother_kws = {
                'window_length': 867,  # Length of the filter window (must be odd)
        ...
        X_dot_derivs.append(X_dot_deriv)
        
        X_dot_deriv_df = pd.DataFrame(X_dot_deriv, columns=derivative_state_variable_names)
        # X_dot_deriv_dfs.append(X_dot_deriv_df)
        
        # Plot the true and derived derivatives for the current VR
        if args.generate_derivative_plots:
            fig, axs = plot_full(t0, X_dot_true_df, feature_names=derivative_state_variable_names, vr=vr, X_pred_df=X_dot_deriv_df, xlim='auto', t_stat_start=0)
            plot_filename = f"derivative_comparison_vr{vr}.png"
            ...
            for i in range(args.num_closeups):
                if args.closeup_xlims is not None and len(args.closeup_xlims) >= 2 * (i + 1):
                    closeup_xlim = args.closeup_xlims[2 * i: 2 * i + 2]
                    fig, axs = plot_full(t0, X_dot_true_df, feature_names=derivative_state_variable_names, vr=vr, X_pred_df=X_dot_deriv_df, xlim=closeup_xlim, t_stat_start=closeup_xlim[0], t_stat_end=closeup_xlim[1])
                    closeup_plot_filename = f"derivative_comparison_vr{vr}_closeup{i+1}.png"
    ...
    print(f"Performing parametric SINDy for VR values: {vr_list}...")
    ...
    overall_feature_library.fit(x=np.zeros_like(np.concat([Xs[0], Us[0]], axis=-1)))  # Fit the library with dummy data to initialize it
    ...
    if args.optimizer == 'STLSQ':
        ...
        # Constraint 1: h' = hdot
        target_hdot_index = derivative_state_variable_names.index('\\dot{h}_0')
        feature_hdot_index = overall_features.index('1 \\dot{h}_0')
        for j in range(n_features):
            pin(target_hdot_index, j, 1.0 if j == feature_hdot_index else 0.0)
        ...
        target_hddot_index = derivative_state_variable_names.index('\\ddot{h}_0')
        
        if '\\ddot{h}_0' in state_variable_names:
            feature_hddot_index = overall_features.index('1 \\ddot{h}_0')
            for j in range(n_features):
                pin(target_hddot_index, j, 1.0 if j == feature_hddot_index else 0.0)
        else:
            feature_h_index = overall_features.index('1 h_0')
            feature_vr2cl_index = overall_features.index('V_R^2 C_{L0}')
            for j in range(n_features):
                if j == feature_h_index:
                    pin(target_hddot_index, j, -4 * np.pi**2)
                elif j == feature_vr2cl_index:
                    pin(target_hddot_index, j, +2 / (np.pi * MSTAR_TRUE))
                else:
                    pin(target_hddot_index, j, 0.0)
    ...
    print("Fitting parametric SINDy model...")
    model = ps.SINDy(
        feature_library=overall_feature_library,
        optimizer=optimizer,
        discrete_time=False
    )
    model.fit(x=Xs, u=Us, t=scaled_t0s, feature_names=overall_variable_names, x_dot=X_dot_derivs)
    ...
    coefficients_filename = f"coefficients_single_parametric.csv"
    ...
    true_nrmse_dict = {
        ...
        } for derivative_state_variable_name in X_dot_true_dfs[0].columns
    }
    ...
    for vr, scaled_t0, X, U, X_dot_true_df, X_dot_deriv in zip(vr_list, scaled_t0s, Xs, Us, X_dot_true_dfs, X_dot_derivs):
        ...
        print(f"Computing true NRMSE for VR = {vr}...")
        for derivative_state_variable_name in X_dot_true_df.columns:
            ...
            true_nrmse_dict[derivative_state_variable_name][vr] = true_nrmse
            
            print(f"NRMSE for {derivative_state_variable_name} at VR = {vr}: {true_nrmse:.2f}%")
        
        print(f"Computing semi NRMSE for VR = {vr}...")
        for derivative_state_variable_name in derivative_state_variable_names:
            ...
            semi_nrmse_dict[derivative_state_variable_name][vr] = semi_nrmse
            
            print(f"NRMSE for {derivative_state_variable_name} at VR = {vr}: {semi_nrmse:.2f}%")
        
        # Generate plots for the current VR
        fig, axs = plot_full(scaled_t0 * vr, X_dot_deriv_df, feature_names=derivative_state_variable_names, vr=vr, X_pred_df=X_dot_pred_df, xlim='auto', t_stat_start=0, nrmse_dict=semi_nrmse_dict)
        plot_filename = f"prediction_derivative_comparison_vr{vr}.png"
        ...
        for i in range(args.num_closeups):
            if args.closeup_xlims is not None and len(args.closeup_xlims) >= 2 * (i + 1):
                closeup_xlim = args.closeup_xlims[2 * i: 2 * i + 2]
                fig, axs = plot_full(scaled_t0 * vr, X_dot_deriv_df, feature_names=derivative_state_variable_names, vr=vr, X_pred_df=X_dot_pred_df, xlim=closeup_xlim, t_stat_start=closeup_xlim[0], t_stat_end=closeup_xlim[1], nrmse_dict=semi_nrmse_dict)
                closeup_plot_filename = f"prediction_derivative_comparison_vr{vr}_closeup{i+1}.png"
    ...
    for vr in vr_list:
        row = {'vr': vr}
        for derivative_state_variable_name in X_dot_true_df.columns:
            row[derivative_state_variable_name] = true_nrmse_dict[derivative_state_variable_name][vr]
        true_nrmse_records.append(row)
        
        row = {'vr': vr}
        for derivative_state_variable_name in derivative_state_variable_names:
            row[derivative_state_variable_name] = semi_nrmse_dict[derivative_state_variable_name][vr]
        semi_nrmse_records.append(row)

    true_nrmse_df = pd.DataFrame(true_nrmse_records)
    true_nrmse_filename = f"true_nrmse_single_parametric.csv"
    ...
    semi_nrmse_filename = f"semi_nrmse_single_parametric.csv"
