...
from sindy_viv.case_bryan.prepare_sindy.input import prepare_input_two_tandem_parametric_vr
from sindy_viv.sindy.functions import perform_sindy
from sindy_viv.utils import add_dot_text, initiate_object, load_config

def parse_args():
    argument_parser = argparse.ArgumentParser(description="Evaluate SINDy on Bryan's LBM data of two tandem oscillating cylinders for all VR and L/D cases, parametrically on VR for each L/D.")
...
def main():
    ...
    for lpd in args.lpd_list:
        ...
        t_s, X_s, X_dot_s, chosen_state_variables, U_s, chosen_input_variables = prepare_input_two_tandem_parametric_vr(
            times=time_dict,
